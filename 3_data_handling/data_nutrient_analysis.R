...
cols.factor = c(
  ...
  "bodymass_error_type" ,
  "bodymass_unit",
  "bodymass_weight_type" ,
  "age_error_type" ,
  "age_unit",
  "species_latin_name_gbif",
  "gbif_id",
  "gbif_id2",
  "environment" ,
  "food_ID",
...
data_source = data.frame(table(data_source$reference_year))
names(data_source) = c("reference_year", "reference_count")
data_source$reference_year <- as.Date(data_source$reference_year, format =
                                        "%Y")
...
data_sampletype = data.frame(table(data_nutrient$sample_type))
names(data_sampletype) = c("sample_type", "count")

coul <- brewer.pal(5, "Set2")
barplot(
  height = data_sampletype$count,
  names = data_sampletype$sample_type,
  col = coul,
  xlab = "Type of sample",
  ylab = "Number of data"
)
...
data_datatype = data.frame(table(data_nutrient$component_data_type))
names(data_datatype) = c("data_type", "count")

coul <- brewer.pal(5, "Set2")
barplot(
  height = data_datatype$count,
  names = data_datatype$data_type,
  col = coul,
  xlab = "Type of data",
  ylab = "Number of data"
)
...
data_samplingenv = data.frame(table(data_nutrient$environment))
names(data_samplingenv) = c("sampling_environment", "count")

coul <- brewer.pal(5, "Set2")
barplot(
  height = data_samplingenv$count,
  names = data_samplingenv$sampling_environment,
  col = coul,
...
tot_dej = length(which(
  !is.na(data_nutrient$feces_ID) |
    !is.na(data_nutrient$urine_ID)
)) #4985

# Proportion of dejection data that also have a coupling with food

dej_food = length(which((
  !is.na(data_nutrient$feces_ID) |
    !is.na(data_nutrient$urine_ID)
) & !is.na(data_nutrient$food_ID))) #1537
dej_food / tot_dej
# Proportion of dejection data that also have a coupling with body

dej_body = length(which((
  !is.na(data_nutrient$feces_ID) |
    !is.na(data_nutrient$urine_ID)
) & !is.na(data_nutrient$body_ID))) #1537
dej_body / tot_dej

# Proportion of dejection data that also have a coupling with food and body

dej_food_body = length(which((
  !is.na(data_nutrient$feces_ID) |
    !is.na(data_nutrient$urine_ID)
) &
  !is.na(data_nutrient$body_ID) &
  !is.na(data_nutrient$food_ID)
)) #1537
...
data_spbodymass = aggregate(
  data_nutrient[, c("bodymass_mean", "species_latin_name_gbif")],
  by = list(species = data_nutrient$species_latin_name_gbif),
  mean,
  simplify = TRUE,
  drop = TRUE
)
...
data_nutrient$observation_resolution[data_nutrient$observation_resolution ==
                                       "inter_indiv"] = "inter_individual"
data_nutrient$observation_resolution[data_nutrient$observation_resolution ==
                                       "intra_indiv"] = "intra_individual"
data_nutrient$observation_resolution = droplevels(data_nutrient$observation_resolution)

data_obsres = data.frame(table(data_nutrient$observation_resolution))
names(data_obsres) = c("observation_resolution", "count")
coul <- brewer.pal(5, "Set2")
barplot(
  height = data_obsres$count,
  names = c(
    "food",
    "individual" ,
    "inter individual",
...
data_chem = data.frame(table(data_nutrient$component_name))
names(data_chem) = c("chemical_component", "count")
...
coul <- brewer.pal(5, "Set2")
barplot(
  height = data_chem$count,
  names = data_chem$observation_resolution,
  col = coul,
  xlab = "Observation resolution",
  ylab = "Number of data"
)
