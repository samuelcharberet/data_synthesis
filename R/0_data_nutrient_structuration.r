...
# fichiers xls(x).
files <-
  list.files(path = here:here("1_data", "1_data_nutrient", "1_data_literature", "1_data"), pattern = "[0-9]{4}_[0-9]{3}_.*?_[0-9]{4}.xls", recursive = TRUE)
...
###### 5. Finding where there are no GBIF_ID in the database ###
...
not_species_id = not_species_id[-is.na(not_species_id)]
...
list_sp <-
  unique(data_nutrient$species_latin_name_gbif) # There are x different species names in the raw nutrient database
...
##### 9. Finding the taxonomic rank of each line ####
...
if (is.null(data_no_match)) {
  print("There are no GBIF species key present in data_nutrient and absent from data_gbif")
}
