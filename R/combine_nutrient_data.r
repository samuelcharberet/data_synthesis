...
combine_nutrient_data = function(data_nl, data_np) {
  ...
  if (length(subspecies) >= 1) {
    message(
      "⚠️  The database nutrients contains subspecies names instead of species binomial names.\n"
    )
    
    message("❗ The following tables contain subspecies ID(s):")
    print(unique(data_nutrients[which(data_nutrients$species_latin_name_gbif == subspecies), ]$reference_ID))
    
    message("\n❗ The following subspecies were found")
    print(subspecies)
  ...
  if (length(synonym) >= 1) {
    message("⚠️  The database nutrients contains synonym species names.\n")
    
    message("❗ The following tables contain synonym ID(s):")
  ...
  if (length(fuzzy) >= 1) {
    message("⚠️  The database nutrients contains fuzzy species names.\n")
    
    message("❗ The following tables contain fuzzy species name(s):")
    print(unique(data_nutrients[which(data_nutrients$species_latin_name_gbif == fuzzy), ]$reference_ID))
    
    message("\n❗ The following fuzzy species names were found")
    print(fuzzy)
  ...
  for (i in 1:nrow(data_nutrients)) {
    if (!is.na(data_nutrients$species_latin_name_gbif[i])) {
      data_nutrients[i , c(
        ...
        useful_data_gbif[which(useful_data_gbif$species == data_nutrients$species_latin_name_gbif[i]), c(
          "canonical_name",
          "rank",
          "status",
          "match_type",
  ...
  if (update == T) {
    ...
    saveRDS(
      taxize_classes,
      file = here::here("1_data", "4_data_taxonomy", "taxize_classes.RData")
    )
  }
