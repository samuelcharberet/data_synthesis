...
load_dnl = function(paths) {
  ...
  # Check that all tables have the same structure ######
  
  ## Do all tables have the same number of column ####
  ...
  if (length(wrong_ncol) >  0) {
    ...
    stop("Not all files in the folder 1_data/1_data_nutrients/1_data_literature/1_data have the correct number of columns")
  }
  rm(list_ncol, wrong_ncol)
  
  
  ## Do all tables have the same names of columns and in the same order ? ####
  ...
  for (i in 1:ncol_database) {
    for (j in 1:n_files) {
      if (list_colnames[[1]][i] != list_colnames[[j]][i]) {
        print(paths[j])
        print(i)
        stop("Not all files in the folder 1_data/1_data_nutrients/1_data_literature/1_data have the same names of columns in the same order")
      }
    }
  }
